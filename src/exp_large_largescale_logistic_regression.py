...
para1 = PARAS(num_agent=20, n=64000, dim=64, lamb1=1/(20*64000), lamb2=64000*1e-6, n_iter=1500, tau=250, batch_size=8192,
              stepsize_LSVRG=1.1, stepsize_SAGA=0.7, stepsize_NIDS=0.15, stepsize_ED=0.05, nmix_LSVRG=28, nmix_SAGA=21)

para2 = PARAS(num_agent=20, n=64000, dim=64, lamb1=1/(20*64000), lamb2=64000*1e-8, n_iter=1800, tau=250, batch_size=8192,
              stepsize_LSVRG=6.3, stepsize_SAGA=4.1, stepsize_NIDS=0.5, stepsize_ED=0.3, nmix_LSVRG=35, nmix_SAGA=28)
paras = [para1, para2]
...
for run in range(len(paras)):
    ...
    optimizers = [
        PMGT_LSVRG_FastMix(problem, 150, x_0, paras[run].stepsize_LSVRG, W, paras[run].nmix_LSVRG,
        ...
        Prox_ED(problem, W, x_0, paras[run].n_iter*6, stepsize=paras[run].stepsize_ED)
    ]
    run_single_exp(optimizers, name='kappa002_largescale_' + str(paras[run].lamb2 / 64000)[-2:])
