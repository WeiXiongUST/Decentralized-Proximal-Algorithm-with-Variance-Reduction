...
para1 = PARAS(num_agent=20, n=6400, dim=64, lamb1=1/(20*6400), lamb2=6400*1e-5, n_iter=300, tau=250, batch_size=1024,
              stepsize_LSVRG=1.15, stepsize_SAGA=0.75, stepsize_NIDS=0.8, stepsize_ED=0.8, nmix_LSVRG=5, nmix_SAGA=3)

para2 = PARAS(num_agent=20, n=6400, dim=64, lamb1=1/(20*6400), lamb2=6400*1e-7, n_iter=300, tau=250, batch_size=1024,
              stepsize_LSVRG=6.1, stepsize_SAGA=4.3, stepsize_NIDS=2.9, stepsize_ED=2.9, nmix_LSVRG=5, nmix_SAGA=5)
paras = [para1, para2]
...
for run in range(len(paras)):
    ...
    run_single_exp(optimizers, name='kappa1_' + str(paras[run].lamb2 / 6400)[-2:])
